...
bool OpenSprinkler::load_hardware_mac(unsigned char *buffer, bool wired)
{
	...
	if (Wire.requestFrom(MAC_CTRL_ID, 6) != 6)
		return false; // if not enough data, return false
...
unsigned char OpenSprinkler::start_ether()
{
#if defined(ESP8266)
	if (hw_rev < 2)
		return 0;																	// ethernet capability is only available when hw_rev>=2
	...
	if (eth.isW5500)
	{
		...
		digitalWrite(PIN_ETHER_CS, HIGH);
		if (ret != 0)
			return 0; // ret is expected to be 0
	}
	else
	{
...
#define ECON1 0x1f
		...
		digitalWrite(PIN_ETHER_CS, HIGH);
		if (r == 0 || r == 255)
			return 0; // r is expected to be a non-255 revision number
	}

	load_hardware_mac((uint8_t *)tmp_buffer, true);
	if (iopts[IOPT_USE_DHCP] == 0)
	{ // config static IP before calling eth.begin
	...
	while (!eth.connected() && (long)(millis() - timeout) < 0)
	{ // overflow proof
		DEBUG_PRINT(".");
		lcd.setCursor(13, 2);
		lcd.print(timecount);
	...
	if (iopts[IOPT_USE_DHCP])
	{
		if (!Ethernet.begin((uint8_t *)tmp_buffer))
			return 0;
...
void OpenSprinkler::begin()
{
	...
	if (detect_i2c(MAIN_I2CADDR))
	{ // check if main PCF8574 exists
	...
	else
	{
		...
		else
		{
			...
			if (detect_i2c(EEPROM_I2CADDR + 2))
			{ // revision 4 has an I2C EEPROM at this address; skipping +1 due to addr conflict with PCF8563
				hw_rev = 4;
			}
			else if (detect_i2c(EEPROM_I2CADDR))
			{ // revision 3 has an I2C EEPROM at this address
				hw_rev = 3;
			}
			else
			{
...
void OpenSprinkler::latch_boost(unsigned char volt)
{
	...
	else
	{
		...
		if (analogRead(PIN_CURR_SENSE) >= top)
			return; // if the voltage has already reached top, return right away
		...
		while ((long)(millis() - boost_timeout) < 0 && analogRead(PIN_CURR_SENSE) < top)
		{ // overflow proof
			delay(5);
		}
		digitalWriteExt(PIN_BOOST, LOW);
...
void OpenSprinkler::latch_setallzonepins(unsigned char value)
{
	...
	{																								 // LATCH contorller only uses PCA9555, no other type
		uint16_t reg = drio->i2c_read(NXP_OUTPUT_REG); // read current output reg value
		if (value)
			reg |= 0x00FF; // first 8 zones are the lowest 8 bits of main driver board
		else
			reg &= 0xFF00;
...
{
	if (A == HIGH && K == HIGH)
		return; // A and K must not be HIGH at the same time
	...
	{ // on main controller
		...
		if (A)
			reg |= (1 << sid); // lowest 8 bits of 9555 control output anodes
		else
			reg &= (~(1 << sid));
...
void OpenSprinkler::apply_all_station_bits(void (*post_activation_callback)())
{
	...
	if (post_activation_callback)
		post_activation_callback();
...
uint16_t OpenSprinkler::read_current()
{
	...
	if (scale < 0)
	{ // assign scale upon first call of this function
		if (hw_type == HW_TYPE_DC)
		{
#if defined(ESP8266)
			scale = 4.88;
...
int OpenSprinkler::detect_exp()
{
	...
	for (n = 4; n >= 0; n--)
	{
		if (detect_i2c(EXP_I2CADDR_BASE + n))
			break;
	}
	return (n + 1) * 2;
#else
...
void OpenSprinkler::set_station_name(unsigned char sid, char tmp[])
{
	...
	if (len != strlen(tmp) || memcmp(n0, tmp, len) != 0)
	{ // only write if the name has changed
...
void OpenSprinkler::switch_special_station(unsigned char sid, unsigned char value, uint16_t dur)
{
	...
	if (!(os.attrib_spe[bid] & (1 << s)))
		return; // if this is not a special stations
...
{
	RFStationCode code;
	if (!parse_rfstation_code(data, &code))
		return; // return if the timing parameter is 0

	if (PIN_RFTX == 255)
		return; // ignore RF station if RF pin disabled
...
#define HTTP_CONNECT_NTRIES 3
	...
	while (true)
	{
		...
		if (nbytes > 0)
		{
			if (pos + nbytes > ETHER_BUFFER_SIZE)
				nbytes = ETHER_BUFFER_SIZE - pos; // cannot read more than buffer size
...
void OpenSprinkler::options_setup()
{
	...
	else
	{
		...
		if (tmp_buffer[0] != 0)
		{
			...
			if (mac != NULL && isValidMAC(tmp_buffer))
			{						// check if bssid is valid MAC
...
{
	if (maxlen > MAX_SOPTS_SIZE)
		maxlen = MAX_SOPTS_SIZE; // cap maxlen
...
void OpenSprinkler::ui_set_options(int oid)
{
	...
	while (!finished)
	{
		...
		case BUTTON_1:
			...
			if (pgm_read_byte(iopt_max + i) != iopts[i])
				iopts[i]++;
			break;
		...
		case BUTTON_3:
			...
			else
			{
				...
				if (i == IOPT_USE_DHCP && iopts[i])
					i += 9; // if use DHCP, skip static ip set
				else if (i == IOPT_HTTPPORT_0)
					i += 2; // skip IOPT_HTTPPORT_1
				else if (i == IOPT_PULSE_RATE_0)
					i += 2; // skip IOPT_PULSE_RATE_1
				else if (i == IOPT_MASTER_STATION && iopts[i] == 0)
					i += 3; // if not using master station, skip master on/off adjust including two retired options
				else if (i == IOPT_MASTER_STATION_2 && iopts[i] == 0)
					i += 3; // if not using master2, skip master2 on/off adjust
				else
				{
					i = (i + 1) % NUM_IOPTS;
				}
				if (i == IOPT_SEQUENTIAL_RETIRED)
					i++;
				if (i == IOPT_URS_RETIRED)
					i++;
				if (i == IOPT_RSO_RETIRED)
					i++;
				if (hw_type == HW_TYPE_AC && i == IOPT_BOOST_TIME)
					i++; // skip boost time for non-DC controller
				if (i == IOPT_LATCH_ON_VOLTAGE && hw_type != HW_TYPE_LATCH)
					i += 2; // skip latch voltage defs for non-latch controllers
#if defined(ESP8266)
				else if (lcd.type() == LCD_I2C && i == IOPT_LCD_CONTRAST)
					i += 3;
#else
				else if (lcd.type() == LCD_I2C && i == IOPT_LCD_CONTRAST)
					i += 2;
...
void OpenSprinkler::lcd_set_brightness(unsigned char value)
{
	...
	else
	{
		...
		{
			lcd.displayOn();
			lcd.setBrightness(iopts[IOPT_LCD_DIMMING]);
		}
	}
#endif
}
